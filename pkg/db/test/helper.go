	...
	*models.GetIngredients() = ing
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 3; j++ {
			...
			branch.Image = faker.DomainName()
			branch.WorkingHour.Open = "10:00"
			branch.WorkingHour.Close = "19:00"
	...
	for i := 0; i < 3; i++ {
		...
		coordinate := models.NewCoordinate(faker.Word(), rand.Float64(), rand.Float64())
